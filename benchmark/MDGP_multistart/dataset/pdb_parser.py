...
# -----------------------------------------------------------------------------------------------------
'''def compute_hydrogen_proximity_pairs(input_file, pdb_id, model, chain, hc_order, cut, local_interval_width, interval_width, output_dir):
#def compute_hydrogen_proximity_pairs(input_file, pdb_id, model, chain, hc_order, output_dir):
	df = pd.read_csv(input_file, sep="\t")
	df.columns = df.columns.str.strip()
	...
	for i in range(len(hydrogens)):
		...
		for j in range(i + 1, len(hydrogens)):
			...
			# fixed lower bound
			dl = 1.8 + np.random.uniform(0, 0.01)

			if d < 2.5:
				# strong NOE
				du = 2.5 + np.random.uniform(0, 0.2)
			elif d < 3.5:
				# medium NOE
				du = 3.5 + np.random.uniform(0, 0.3)
			elif d < 5.0:
				# weak NOE
				du = 5.0 + np.random.uniform(0, 0.4)
			elif d < 6.0:
				# very weak NOE
				du = 6.0 + np.random.uniform(0, 0.5)
			else:
				# skip pairs beyond NOE range
				continue
			
			# Ensure ordering by atom_id_i > atom_id_j
			if ai > aj:
				pair_list.append((ai, aj, ri, rj, dl, du, ni, nj, rni, rnj))
			else:
				pair_list.append((aj, ai, rj, ri, dl, du, nj, ni, rnj, rni))

	# Sort pairs by atom_id_i
	pair_list.sort(key=lambda x: x[0])
	...
	print(f"Step 06: hydrogen-hydrogen NOE distances saved to: {output_file}")
'''# -----------------------------------------------------------------------------------------------------
...
def merge_intervals_from_output(pdb_id, model, chain, hc_order, output_dir):
    ...
    results = []
    for key, group in grouped:
        # Check if any row has a fixed interval (d_l == d_u)
        fixed_intervals = group[group["d_l"] == group["d_u"]]

        if not fixed_intervals.empty:
            # Keep the smallest fixed value as both d_l and d_u
            d_fixed = fixed_intervals["d_l"].min()
            dl, du = d_fixed, d_fixed
        else:
            # Default behavior: merge by max(d_l) and min(d_u)
            dl = group["d_l"].max()
            du = group["d_u"].min()
...
# -----------------------------------------------------------------------------------------------------
def vertex_from_atom(vertices_atoms_map, residue0, atom):
	"""
	...
	residue = int(vertices_atoms_map.iloc[0]['res_i']) + residue0 - 1
...
# -----------------------------------------------------------------------------------------------------
def ddgp_cliques(ddgp_hc_order_vec, vertices_atoms_map, n):
	"""
	Generates the clique matrix used for DDGP modeling, allowing different patterns per residue.
...
def save_T_file(output_dir, pdb_id, model, chain, hc_order, ddgp_hc_order_vec, angular_width):
	...
	if vertices_map.iloc[0]['atom_i'] == 'CA':
		new_row = pd.DataFrame([{'i': 1, 'res_i': df.iloc[0]['res_i'], 'atom_i': 'N'}])
		vertices_map = pd.concat([new_row, vertices_map], ignore_index=True)
	
	# Generate T0 matrix (ensure it is integer type)
	if len(ddgp_hc_order_vec) > 1:
		T0 = ddgp_cliques(ddgp_hc_order_vec, vertices_map, n)
